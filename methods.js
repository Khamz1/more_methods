...
const newO = array.filter((element)=>
   element.id!==id)
...
function getUsersGeo(array, id) {
  const newObj = array.find((element)=>
    element.id===id)
    
    console.log(newObj.address.geo)
  }
...
function getUsersCompany(array, id) {
  const newO = array.find((el)=>
  el.id===id)
    console.log(newO.company)
...
function changePhone(array, id, phone) {
  const newAr=array.filter((el)=>
   el.id==id? el.phone=phone:null)
 console.log(newAr)
 }
 changePhone(users, 2, 423424)